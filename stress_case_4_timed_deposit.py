...
def stress_case_4_timed_deposit(model):
    ...
    for week in range(1, model.loan_tenor + 1):
        ...
        if week == model.timed_deposit_shock_weeks:
            info_array[stressed_deposit, week] = model.account_balance_base_case.info_array[1, week] * \
                                                 model.timed_deposit_shock
        else:
            info_array[stressed_deposit, week] = model.account_balance_base_case.info_array[1, week]

        info_array[expense, week] = - min(model.avg_weekly_expense_3mth, info_array[:expense, week].sum())
